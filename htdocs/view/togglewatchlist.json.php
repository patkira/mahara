...
if (get_record('usr_watchlist_view', 'usr', $data->usr, 'view', $viewid)) {
    ...
    if ($artefact) {
        $result->newtext = '<span class="icon icon-eye prs"></span>'. get_string('addtowatchlistartefact', 'view', $title);
    }
    else {
        $result->newtext = '<span class="icon icon-eye prs"></span>' . get_string('addtowatchlist', 'view');
    }
...
if ($artefact) {
    $result->newtext = '<span class="icon icon-eye-slash prs"></span>' . get_string('removefromwatchlistartefact', 'view', $title);
}
else {
    $result->newtext = '<span class="icon icon-eye-slash prs"></span>'. get_string('removefromwatchlist', 'view');
}
$result->watched = true;
json_reply(false, $result);
